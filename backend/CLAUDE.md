...
- `searxng/` - Optional self-hosted metasearch provider for the `web_search` tool; reads `base_url` from config or `SEARXNG_BASE_URL` and normalizes JSON results into DeerFlow's `{title,url,content}` shape
